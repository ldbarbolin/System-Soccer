...
class Usuario{
    ...
    async seleccionarUsuarioPorNombreContrasena(nombreUsuario, contrasena) {
        try {
          ...
          const consulta = "SELECT u.id AS id_usuario, p.id AS id_persona, u.id_tipo_usuario,u.nombreUsuario, p.nombre, p.apellido_paterno, p.apellido_materno, p.ci, p.genero, tu.tipo AS tipo_usuario FROM usuario u JOIN persona p ON u.id = p.id JOIN tipousuario tu ON u.id_tipo_usuario = tu.id WHERE u.nombreUsuario = ? AND u.contrasena = ? AND u.estadoU = 'ACT';"
